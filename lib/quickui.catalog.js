...
BrowserSpecific.prototype.extend({
    ...
    initialize: function() {
        ...
        if ( Control.browser.opera ) {
            content = this.opera();
        }
        if ( Control.browser.webkit ) {
            content = this.webkit();
        }
...
CalendarDay.prototype.extend({
    ...
    initialize: function() {
        ...
        if ( !this.date() ) {
            this.date( this._defaultDate() );
        }
    },

    _defaultDate: function() {
...
LabeledInput.prototype.extend({
    ...
    _type: function( type ) {
        ...
        if ( type !== undefined && Control.browser.msie && parseInt( Control.browser.version ) < 9 ) {
...
Popup.prototype.extend({
    ...
    _isElementOutsideControl: function( element ) {
        var elementIsControl = ( this.index( element ) >= 0 );
        var elementInsideControl = ( $( element ).parents().filter( this ).length > 0 );
    ...
    _bindContentEvents: function() {
        ...
        this.$ComboBox_content().on({
            ...
            "keydown": function( event ) {
                ...
                if ( event.which === 13 && opened && self.closeOnEnter() ) {
...
Menu.prototype.extend({
    ...
    _updateShield: function() {
        ...
        var shieldWidth = $content.width()
            + parseFloat( $content.css( "padding-left" ) )
...
(function() {
  ...
  window.TextBoxWithButton = (function(_super) {
    ...
    function TextBoxWithButton() {
      return TextBoxWithButton.__super__.constructor.apply(this, arguments);
    }
